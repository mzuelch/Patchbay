...
def build_tab_input(self: "MainWindow") -> None:
    ...
    """
    dpg = self.dpg
    with dpg.tab(label="Input & Anchors"):
        # Row 1: input load
        ...
        with dpg.table(
            ...
            with dpg.table_row():
                with dpg.group() as g_left:
                    ...
                    self._build_waveform_controls_input()
                    self._build_marker_controls()
                    dpg.add_separator()
                    dpg.add_text("Anchors")
                    self._build_anchor_table(width=-1, height=220)

                with dpg.group():
                    dpg.add_text("Input processing (AudioFX)")
                    with dpg.child_window(border=True, height=520, width=-1):
